...
- **Caching.** Cloudflare does not cache HTML by default, so every read today
  ...
  being served. Before the cutover the split is live on the legacy hosts:
  documents on `symposium.site` and the API on `api.symposium.md`. The cutover
  makes `openartifacts.site` and `api.openartifacts.ai` the canonical hosts, with
  no `workers.dev` url at all, since a listing there would take every Worker on
  the account's subdomain with it.
...
| [symposium.site](https://symposium.site) | Legacy document host. Redirects `GET` and `HEAD` with the exact path and query to `openartifacts.site`. |
| [api.symposium.md](https://api.symposium.md) | Retired API host. Every method and path returns `410 Gone` before authentication. |
