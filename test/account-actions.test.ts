...
it("reports account plan, limits, combined usage and association without revealing external identity", async () => {
  ...
  expect(response.status).toBe(200);
  expect(await response.json()).toEqual({ accountId: ACCOUNT, plan: "free", limits: { documents: 1, pushesPerDay: 6, htmlBytes: 1048576 }, usage: { documents: 2, pushesToday: 4, storedBytes: 0 }, externalLinked: true, refresh: { status: "unavailable", checkedAt: null, expiresAt: null } });
